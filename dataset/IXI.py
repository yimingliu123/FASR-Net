...
class TrainSet(Dataset):
    def __init__(self, args, transform=transforms.Compose([RandomFlip(), RandomRotate(), ToTensor()]) ):
        self.input_list = sorted([os.path.join(args.dataset_dir, 'T2', name) for name in 
            os.listdir( os.path.join(args.dataset_dir, 'T2') )])
        self.ref_list = sorted([os.path.join(args.dataset_dir, 'PD', name) for name in 
            os.listdir( os.path.join(args.dataset_dir, 'PD') )])
...
class TestSet(Dataset):
    def __init__(self, args, ref_level='1', transform=transforms.Compose([ToTensor()])):
        self.input_list = sorted([os.path.join(args.dataset_dir, 'T2_test', name) for name in 
            os.listdir( os.path.join(args.dataset_dir, 'T2_test') )])[:100]
        self.ref_list = sorted([os.path.join(args.dataset_dir, 'PD_test', name) for name in 
            os.listdir( os.path.join(args.dataset_dir, 'PD_test') )])[:100]
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            sample = self.transform(sample)
        return sample, self.input_list[idx]